...
c (t_i in Dunlap)
c
c      call aint_query_gpu_level(igpu)
c     if (igpu.gt.2) then
c        call aint_coulomb_init()
c      endif
c      if (igpu.eq.5) MEMO = .false.
      !MEMO=.true.
c      if (MEMO) then
c         call g2g_timer_start('int3mem')
c         call g2g_timer_sum_start('Coulomb precalc')
c Large elements of t_i put into double-precision cool here
c Size criteria based on size of pre-factor in Gaussian Product Theorem
c (applied to MO basis indices)
c         call int3mem()
c Small elements of t_i put into single-precision cools here
c         call int3mems()
c         call g2g_timer_stop('int3mem')
c         call g2g_timer_sum_stop('Coulomb precalc')
c      endif
c#ifdef CUBLAS
c            if(.not.TRANSPORT_CALC) call CUBLAS_FREE(devPtrY)
c#endif

            call g2g_timer_stop('inicio')
!TRANSPORT
c          if(TRANSPORT_CALC) then
c             traza10=0
c             traza100=0
c             alfa=1.0D0
!            GammaMagnus=0.036285
...
!            GammaMagnus=0.00184 ! GammaMagnus es la Gamma que se usa en los pasos de Magnus
!            GammaMagnus=0.0048380
c             GammaMagnus=driving_rate
!            GammaMagnus=0.012
!            GammaMagnus=0.05
c             GammaVerlet=GammaMagnus*0.1 ! GammaVerlet se usa en los primeros 200 pasos en los que se hace Verlet
c             write(*,*) ' Driving Rate =', GammaMagnus
c           endif
!
c            call g2g_timer_stop('inicio')
!##############################################################################!
...
!            field=.false. juanderboy
         if(GATEFIELD) then
              if((istep.le.15000).and.(istep.gt.10000))  then
                 ...
                 g=1.0D0
                 factor=2.54D0
                 fxx=fx*exp(-0.00001*(dble(istep-15000))**2)
                 fyy=fy*exp(-0.00001*(dble(istep-15000))**2)
                 fzz=fz*exp(-0.00001*(dble(istep-15000))**2)
              elseif(istep.gt.15000) then
                 call dip(ux,uy,uz)
                 g=1.0D0
                 factor=2.54D0
                 fxx=fx
